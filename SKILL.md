...
Work under a persistent cache outside the repository unless the user requests another location:

```bash
export TDP_SKILL_DIR="<absolute directory containing this SKILL.md>"
export TDP_CACHE="${XDG_CACHE_HOME:-$HOME/.cache}/3gpp-review"
mkdir -p "$TDP_CACHE"
python3 -m venv "$TDP_CACHE/venv"
"$TDP_CACHE/venv/bin/python" -m pip install --upgrade pip
"$TDP_CACHE/venv/bin/python" -m pip install openpyxl lxml
```
...
If network access or package installation is unavailable, continue from user-provided local Index/TDoc files when possible and state the missing capability.
...
```bash
PY="$TDP_CACHE/venv/bin/python"
"$PY" "$TDP_SKILL_DIR/scripts/3gpp_tdocs.py" inspect-index \
...
```bash
"$PY" "$TDP_SKILL_DIR/scripts/3gpp_tdocs.py" filter-index \
  --excel "$meeting_cache/index/extracted/<index.xlsx>" \
  --sheet '<sheet name>' \
  --agenda '20.6.22' \
...
```bash
"$PY" "$TDP_SKILL_DIR/scripts/3gpp_tdocs.py" download \
...
```bash
"$PY" "$TDP_SKILL_DIR/scripts/3gpp_tdocs.py" extract \
...
```bash
"$PY" "$TDP_SKILL_DIR/scripts/3gpp_tdocs.py" coverage \
  --manifest "$meeting_cache/proposals.json" \
  --texts "$meeting_cache/texts"
```
...
Write the final Markdown to the repository or path requested by the user. Otherwise use a descriptive filename such as `<meeting>_KI<ki>.md` in the current working directory.
